    ...
    return new Promise(function(resolve,reject){ 
        ...
        if (state){
            getCode(state).then(function(v){
                code = v['State Code']
                query = 'select * from accident join vehicle ON (accident."YEAR" = vehicle."ACC_YEAR" and vehicle."ST_CASE" = accident."ST_CASE") where (accident."MONTH" ='+month+' and accident."YEAR"='+year+' and accident."DAY"='+day+') and (vehicle."VIN" like \'4YD%\' or vehicle."VIN" like \'1NL%\' or vehicle."VIN" like \'1RF%\' or vehicle."VIN" like \'4SL%\' or vehicle."VIN" like \'4VZ%\' or vehicle."VIN" like \'5B4%\' or vehicle."VIN" like \'4S7%\' or vehicle."VIN" like \'4VZ%\' or vehicle."VIN" like \'1KB%\' or vehicle."VIN" like \'4UZ%\' or vehicle."VIN" like \'4U7%\' or vehicle."VIN" like \'1TC%\');'
                console.log(query)
...
function addMatches(data){
    var promises = _.map(data[0], function(item) {
        return new Promise(function(resolve,reject){
            ...
            insertion =[data[1]["id"],item["vehicle_index"],item["index"],item["ACC_YEAR"],item["STATE"],item["ST_CASE"]]
            


            db.one('insert into vin_make_match_no_date("claim_index","accident_index","vehicle_index","ACC_YEAR","STATE","ST_CASE")' + 'values($1,$2,$3,$4,$5,$6) RETURNING id,claim_index', insertion).then(function(data){
                resolve(data)
            })       
        })
    })
