    ...
    return new Promise(function(resolve,reject){ 
        ...
        if (state){
            getCode(state).then(function(v){
                code = v['State Code']
                query = 'select * from accident join vehicle ON accident."YEAR" = vehicle."ACC_YEAR" and vehicle."ST_CASE" = accident."ST_CASE" join vin on(vin."vehicle_index"=vehicle."index" and vin."ACC_YEAR"::int=vehicle."ACC_YEAR"::int) where  (accident."MONTH" ='+month+' and accident."YEAR"='+year+' and accident."DAY"='+day+' and vehicle."VIN" like \'1FU%\') or (accident."MONTH" ='+month+' and accident."YEAR"='+year+' and accident."DAY"='+day+' and vehicle."VIN" like \'1RF%\') or (accident."MONTH" ='+month+' and accident."YEAR"='+year+' and accident."DAY"='+day+' and vehicle."VIN" like \'4VZ%\')'
...
function addMatches(data){
    var promises = _.map(data[0], function(item) {
        return new Promise(function(resolve,reject){
            ...
            db.one('insert into f_r_match("claim_index","accident_index","vehicle_index","ACC_YEAR","STATE","ST_CASE")' + 'values($1,$2,$3,$4,$5,$6) RETURNING id,claim_index', insertion).then(function(data){
                resolve(data)
            })       
        })
    })
